          ...
          <div class="card">
            <div class="card-body">
              ...
              <div v-if="isActiveTab(2)">
                <Instruction :errors="errors" :upload_attachment_url="upload_attachment_url" @changeTab="changeTab($event)" @submitRequest="handleSubmit($event)"></Instruction>
              </div>
              <!-- Tab 2 Ends -->
            </div>
          </div>
  ...
  methods: {
    ...
    changeTab(tabNumber) {
      if (tabNumber == 2 && !this.user_id) {
        window.location = this.sign_in_url;
        return false;
      }
